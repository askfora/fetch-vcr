...
function buildHash(url, args) {
  ...
  var VCR_SALT = typeof process !== 'undefined' ? process.env['VCR_SALT'] : null
  if (VCR_SALT) json.salt = VCR_SALT;
...
function buildFilenamePrefix(url, args, hash, suffixLength) {
  ...
  var filename = url + paramsHash + '_' + method + '_' + hash;
  var totalLength = filename.length + suffixLength;
  if (totalLength > MAX_FILENAME_LENGTH) {
    var namehash = crypto.createHash('sha256');
    var code = namehash.update(filename);
    var digest = namehash.digest("base64").replace(/[^A-Za-z]/g, "");
